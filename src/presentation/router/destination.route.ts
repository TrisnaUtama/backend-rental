import { destinationService, response, userService } from "./../../application/instances";
...
import type { CreateDestination, UpdateDestination, UpdateFacility } from "../../infrastructure/entity/types";
...
.use(
    ...
    .post('/', async ({ set, body }) => {
        try {
            ...
            const destination = await destinationService.create(payload_x, payload_y);
            
            set.status = 201;
            return StandardResponse.success(destination, "Success creating new destination");
        } catch (error) {
            set.status = 500;
            return GlobalErrorHandler.handleError(error, set);
        }
        ...
        body: t.Object({
            ...
            facilities: t.Array(t.Object({
                ...
                error: "Facilities must be an array of objects with name and status"
            }),
        })
    })
    .patch('/:id', async ({ set, body, params }) => {
        try {
            ...
            const updatedDestination = await destinationService.update(params.id, payload_x, payload_y);
            set.status = 201;
            return StandardResponse.success(updatedDestination, "Success updating destination and facilities");
        } catch (error) {
            set.status = 500;
            return GlobalErrorHandler.handleError(error, set);
        }
        ...
        body: t.Object({
            ...
            facilities: t.Array(t.Object({
                ...
                error: "Facilities must be an array of objects with id, name, and status"
            }),
        })
    })
