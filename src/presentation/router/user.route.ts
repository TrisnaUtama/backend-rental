...
export const userRoute = new Elysia({ prefix: "/v1/users" })
	...
	.get("/", async ({ set }) => {
		try {
			...
			if (!users) {
				set.status = 400;
				throw StandardResponse.error(
					"Something went wrong while retreived users",
					set.status,
				);
			}
	...
	.get("/:id", async ({ set, params }) => {
		try {
			...
			if (!user) {
				set.status = 404;
				throw StandardResponse.error("User not found !", set.status);
			}
	...
	.post(
		...
		async ({ body, set }) => {
			try {
				...
				if (!new_account) {
					set.status = 400;
					throw StandardResponse.error("Error while creating user data");
				}
				set.status = 201;
	...
	.patch(
		...
		async ({ body, params, set }) => {
			try {
				...
				if (!updated_user) {
					set.status = 400;
					throw StandardResponse.error("Error while updating user data");
				}
				set.status = 201;
				return StandardResponse.success(
					updated_user,
