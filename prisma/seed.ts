...
import { prisma } from "../src/infrastructure/utils/prisma"; // Assuming this path is correct
...
async function main() {
  try {
    ...
    const destinations = [
      ...
      {
        "id": "cmbi8isju0002sd5a4qy9sut3", // Assuming this was a typo in your provided data: "cmbi8isju0002sd5a4qy9sut3" instead of "cmbi8isiu0002sd5a4qy9sut3"
        "name": "Balangan Beach",
        "open_hour": "08:00 - 17:00",
        "description": "Perfect for family vacations and relaxing getaways.",
        "image_urls": [
      ...
      try {
        await prisma.destinations.upsert({ 
          ...
          update: {
            ...
            deleted_at: destinationData.deleted_at ? new Date(destinationData.deleted_at) : null,
          },
          create: {
        ...
        console.log(`Seeded destination: ${destinationData.name} (${destinationData.id})`);
      } catch (innerError) {
        if (innerError instanceof PrismaClientKnownRequestError) {
          console.error(`Error seeding destination ${destinationData.name} (${destinationData.id}):`, innerError.message);
        } else {
          console.error(`Unexpected error seeding destination ${destinationData.name} (${destinationData.id}):`, innerError);
        }
      }
    }
