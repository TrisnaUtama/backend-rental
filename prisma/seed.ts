    ...
    for (const customer of customers) {
      ...
      for (const destination of destinationsToRate) {
        try {
          ...
          console.log(`Seeded rating from ${customer.email} for ${destination.name}`);
        } catch (innerError) {
          if (innerError instanceof PrismaClientKnownRequestError && innerError.code === 'P2002') {
            console.warn(`Rating already exists for user ${customer.email} and destination ${destination.name}. Skipping.`);
          } else {
            console.error(`Error seeding rating from ${customer.email} for ${destination.name}:`, innerError);
          }
        }
      }
    }
